...
type HorizonServerConfig struct {
	Cipher      string               `json:"method"`
	Password    string               `json:"password"`
	Level       byte                 `json:"level"`
	Email       string               `json:"email"`
	Users       []*HorizonUserConfig `json:"clients"`
	NetworkList *NetworkList         `json:"network"`
	IVCheck     bool                 `json:"ivCheck"`
}

func (v *HorizonServerConfig) Build() (proto.Message, error) {
	if C.Contains(shadowaead_2022.List, v.Cipher) {
		return buildHorizon(v)
	}
	...
	if v.Users != nil {
		for _, user := range v.Users {
			...
			if account.Password == "" {
				return nil, newError("Horizon password is not specified.")
			}
...
func buildHorizon(v *HorizonServerConfig) (proto.Message, error) {
	if len(v.Users) == 0 {
...
type HorizonServerTarget struct {
	Address    *Address `json:"address"`
	Port       uint16   `json:"port"`
	Cipher     string   `json:"method"`
	Password   string   `json:"password"`
...
type HorizonClientConfig struct {
	Servers []*HorizonServerTarget `json:"servers"`
}

func (v *HorizonClientConfig) Build() (proto.Message, error) {
	if len(v.Servers) == 0 {
		return nil, newError("0 Horizon server configured.")
	}
	...
	if len(v.Servers) == 1 {
		...
		if C.Contains(shadowaead_2022.List, server.Cipher) {
			if server.Address == nil {
				return nil, newError("Horizon server address is not set.")
			}
			if server.Port == 0 {
				return nil, newError("Invalid Horizon port.")
			}
			if server.Password == "" {
				return nil, newError("Horizon password is not specified.")
			}
	...
	for idx, server := range v.Servers {
		if C.Contains(shadowaead_2022.List, server.Cipher) {
			return nil, newError("Horizon 2022 accept no multi servers")
		}
		if server.Address == nil {
			return nil, newError("Horizon server address is not set.")
		}
